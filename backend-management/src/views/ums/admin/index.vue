  ...
  setup() {
    ...
    const methods = reactive({
      ...
      handleAdd(){
        ...
        states.isEdit = false;
        states.admin = Object.assign({},defaultAdmin);
      },

      handleUpdate(index, row){
        states.dialogVisible=true;
        states.isEdit = true;
        states.admin = Object.assign({},row);
      },
      handleDialogConfirm() {
        ElMessageBox.confirm('是否要确认?', '提示', {
          ...
          if (states.isEdit) {
            updateAdmin(states.admin.id,states.admin).then(() => {
              ElMessage({
                message: '修改成功！',
                type: 'success'
              });
      ...
      handleSelectRole(index,row){
        states.allocAdminId = row.id;
        states.allocDialogVisible = true;
        methods.getRoleListByAdmin(row.id);
      },
      handleAllocDialogConfirm(){
        ElMessageBox.confirm('是否要确认?', '提示', {
          ...
          params.append("adminId", states.allocAdminId);
          params.append("roleIds", states.allocRoleIds);
          allocRole(params).then(() => {
            ElMessage({
              message: '分配成功！',
              type: 'success'
            });
            states.allocDialogVisible = false;
          })
        })
      },
      ...
      getAllRoleList() {
        fetchAllRoleList().then(response => {
          states.allRoleList = response.data;
        });
      },
        ...
        getRoleByAdmin(adminId).then(response => {
          ...
          if(allocRoleList!=null&&allocRoleList.length>0){
            for(let i=0;i<allocRoleList.length;i++){
              states.allocRoleIds.push(allocRoleList[i].id);
            }
          }
        });
      },
