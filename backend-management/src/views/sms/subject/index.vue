<template>
  ...
  <!-- 添加专题的对话框 -->
  <el-dialog title="选择专题" v-model="selectDialogVisible" width="50%">
  ...
  setup() {
    ...
    const methods = reactive({
      ...
      handleDelete(index, row) {
        methods.deleteSubject(row.id);
      },

      handleSelectSubject(){
        states.selectDialogVisible=true;
        states.dialogData.listQuery.keyword=null;
        methods.getDialogList();
      },
      getDialogList(){
        fetchSubjectList(states.dialogData.listQuery).then(response=>{
      ...
      handleSelectDialogConfirm(){
        ...
        for (let i = 0; i < states.dialogData.multipleSelection.length; i++) {
          selectSubjects.push({
            subjectId:states.dialogData.multipleSelection[i].id,
            subjectName:states.dialogData.multipleSelection[i].name
          });
        }
      ...
      handleUpdateSort(){
        ElMessageBox.confirm('是否要修改排序?', '提示', {
          ...
          updateHomeSubjectSort(states.sortDialogData).then(()=>{
