...
export default defineComponent({
  ...
  setup() {
    ...
    const methods = reactive({
      ...
      getDialogList(){
        fetchBrandList(states.dialogData.listQuery).then(response=>{
      ...
      handleSelectDialogConfirm(){
        ...
        for (let i = 0; i < states.dialogData.multipleSelection.length; i++) {
          selectBrands.push({
            brandId:states.dialogData.multipleSelection[i].id,
            brandName:states.dialogData.multipleSelection[i].name
          });
        }
      ...
      handleUpdateSort(){
        ElMessageBox.confirm('是否要修改排序?', '提示', {
          ...
          updateHomeBrandSort(states.sortDialogData).then(()=>{
