    ...
    <el-card shadow="never" style="margin-top: 15px">
      <div class="operate-container">
        ...
        <div class="operate-button-container" v-show="order.status===0">
          ...
          <el-button size="small">修改商品信息</el-button>
    ...
    <el-dialog title="修改收货人信息"
      ...
      <el-form :model="receiverInfo"
        ...
        <el-form-item label="所在区域：">
          <distpicker :province="receiverInfo.receiverProvince"
                      :city="receiverInfo.receiverCity"
                      :country="receiverInfo.receiverRegion"
                      @pickerChanged="onSelectRegion"></distpicker>
    ...
    const methods = reactive({
      ...
      getList(id){
        getOrderDetail(id).then(response => {
          Object.assign(states.order, response.data);
        });
      },
      // 选择地区
      onSelectRegion(data){
        states.receiverInfo.receiverProvince=data.province;
        states.receiverInfo.receiverCity=data.city;
        states.receiverInfo.receiverRegion=data.country;
      },
      // 打开修改信息菜单
      showUpdateReceiverDialog(){
        states.receiverDialogVisible=true;
        Object.assign(states.receiverInfo, {
          orderId:states.order.id,
          receiverName:states.order.receiverName,
          receiverPhone:states.order.receiverPhone,
          receiverPostCode:states.order.receiverPostCode,
          receiverDetailAddress:states.order.receiverDetailAddress,
          receiverProvince:states.order.receiverProvince,
          receiverCity:states.order.receiverCity,
          receiverRegion:states.order.receiverRegion,
      ...
      handleUpdateReceiverInfo(){
        ElMessageBox.confirm('是否要修改收货信息?', '提示', {
          ...
          updateReceiverInfo(states.receiverInfo).then(()=>{
            states.receiverDialogVisible=false;
            ElMessage({
              type: 'success',
              message: '修改成功!'
            });
            methods.getList(states.id)
          });
        });
      },
      // 展示费用对话框
      showUpdateMoneyDialog(){
        states.moneyDialogVisible=true;
        states.moneyInfo.orderId=states.order.id;
        states.moneyInfo.freightAmount=states.order.freightAmount;
        states.moneyInfo.discountAmount=states.order.discountAmount;
        states.moneyInfo.status=states.order.status;
      },
      handleUpdateMoneyInfo(){
        ElMessageBox.confirm('是否要修改费用信息?', '提示', {
          ...
          updateMoneyInfo(states.moneyInfo).then(()=>{
            states.moneyDialogVisible=false;
            ElMessage({
              type: 'success',
              message: '修改成功!'
            });
      ...
      // 站内信对话框
      showMessageDialog(){
        states.messageDialogVisible=true;
        states.message.title=null;
        states.message.content=null;
      },
      handleSendMessage(){
        ElMessageBox.confirm('是否要发送站内信?', '提示', {
          ...
          states.messageDialogVisible=false;
          ElMessage({
            type: 'success',
            message: '发送成功!'
          });
      ...
      showCloseOrderDialog(){
        states.closeDialogVisible=true;
        states.closeInfo.note=null;
        states.closeInfo.id=states.id;
      },
      handleCloseOrder(){
        ElMessageBox.confirm('是否要关闭?', '提示', {
          ...
          type: 'warning'
            ...
            params.append("ids",[states.closeInfo.id]);
            params.append("note",states.closeInfo.note);
            closeOrder(params).then(()=>{
              states.closeDialogVisible=false;
              ElMessage({
                type: 'success',
                message: '订单关闭成功!'
              });
      ...
      // 展示备注订单对话框
      showMarkOrderDialog(){
        states.markOrderDialogVisible=true;
        states.markInfo.id=states.id;
        states.markInfo.note=null;
      },
      handleMarkOrder(){
        ElMessageBox.confirm('是否要备注订单?', '提示', {
          ...
          let params = new URLSearchParams();
          params.append("id",states.markInfo.id);
          params.append("note",states.markInfo.note);
          params.append("status",states.order.status);
          updateOrderNote(params).then(()=>{
            states.markOrderDialogVisible=false;
            ElMessage({
              type: 'success',
              message: '订单备注成功!'
            });
