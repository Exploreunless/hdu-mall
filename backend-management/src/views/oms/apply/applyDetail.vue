      ...
      <div class="form-container-border">
        ...
        <el-row>
          ...
          <el-col class="form-border font-small" :span="18" style="height:100px">
            <img v-for="(item, index) in proofPics" style="width:80px;height:80px" :src="item" :key="index">
          </el-col>
        </el-row>
      </div>
      <div class="form-container-border">
...
export default defineComponent({
  ...
  setup() {
    ...
    const totalAmount = computed(()=>{
      if (states.orderReturnApply != null) {
        return states.orderReturnApply.productRealPrice * states.orderReturnApply.productCount;
      } else {
        return 0;
      }
    });
    ...
    const currentAddress = computed(()=>{
      let id = states.updateStatusParam.companyAddressId;
      if(states.companyAddressList==null)return {};
      for (let i = 0; i < states.companyAddressList.length; i++) {
        let address = states.companyAddressList[i];
        if (address.id === id) {
          return address;
        }
      }
    ...
    const methods = reactive({
      handleViewOrder(){
        router.push({path:'/oms/orderDetail',query:{id:states.orderReturnApply.orderId}});
      },
      getDetail() {
        getApplyDetail(states.id).then(response => {
          console.log("getDetail")
          states.orderReturnApply = response.data;
          states.productList = [];
          states.productList.push(states.orderReturnApply);
          if (states.orderReturnApply.proofPics != null) {
            states.proofPics = states.orderReturnApply.proofPics.split(",")
          }
          //退货中和完成
          if(states.orderReturnApply.status===1||states.orderReturnApply.status===2){
            states.updateStatusParam.returnAmount=states.orderReturnApply.returnAmount;
            states.updateStatusParam.companyAddressId=states.orderReturnApply.companyAddressId;
          }
          methods.getCompanyAddressList();
        });
      },
      getCompanyAddressList() {
        fetchList().then(response => {
          states.companyAddressList = response.data;
          for (let i = 0; i < states.companyAddressList.length; i++) {
            if (states.companyAddressList[i].receiveStatus === 1&&states.orderReturnApply.status===0) {
              states.updateStatusParam.companyAddressId = states.companyAddressList[i].id;
            }
          }
        });
      },
      handleUpdateStatus(status){
        states.updateStatusParam.status=status;
        ElMessageBox.confirm('是否要进行此操作?', '提示', {
          confirmButtonText: '确定',
          cancelButtonText: '取消',
          type: 'warning'
        }).then(() => {
          updateApplyStatus(states.id,states.updateStatusParam).then(()=>{
            ElMessage({
              type: 'success',
              message: '操作成功!',
              duration:1000
